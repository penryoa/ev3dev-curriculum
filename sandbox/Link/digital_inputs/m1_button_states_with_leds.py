...
Much like we did in Link of the motors unit, later we will show you different (better) ways to use buttons.
...
def main():
    ...
    while True:
        if btn.up:
            print('up')
            ev3.Leds.set_color(ev3.Leds.BLACK)
        if btn.left:
            print('left')
            ev3.Leds.set_color(ev3.Leds.GREEN)
        if btn.right:
            print('right')
            ev3.Leds.set_color(ev3.Leds.RED)
